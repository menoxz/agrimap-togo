...
def compute_synthesis(
    ...
    level="region"    → charge density.geojson, nom_region
    level="prefecture" → charge density_prefecture.geojson, nom_prefecture
    """
    ...
    density_gdf = load_analysis(str(analyses_dir / f"density{suffix}.geojson"))
    zaap_gdf = load_analysis(str(analyses_dir / f"zaap_coverage{suffix}.geojson"))
    access_gdf = load_analysis(str(analyses_dir / f"accessibility{suffix}.geojson"))
    coop_gdf = load_analysis(str(analyses_dir / f"cooperative_network{suffix}.geojson"))
    
    logger.step(f"  Densité: {len(density_gdf)} · ZAAP: {len(zaap_gdf)} · "
                f"Access: {len(access_gdf)} · Coop: {len(coop_gdf)}")

    # Base des régions
    base_boundaries = density_gdf.copy()

    # Density
    if "density_class" in density_gdf.columns:
        density_scores = density_gdf[[name_col, "density_class", "density"]].copy()
    ...
    if "coverage_class" in zaap_gdf.columns:
        zaap_scores = zaap_gdf[[name_col, "coverage_class", "coverage_pct"]].copy()
    ...
    if "accessibility_score" in access_gdf.columns:
        _access_cols = [name_col, "accessibility_score", "accessibility_class"]
    ...
    if "white_zone_pct" in coop_gdf.columns:
        _coop_cols = [name_col, "coop_class", "white_zone_pct", "coop_density_per_1000km2"]
    ...
    # ── Fusion ────────────────────────────────────────────────────────
    merged = base_boundaries[[name_col, "geometry"]].copy()
    
    if not density_scores.empty:
        merged = merged.merge(density_scores[[name_col, "density_score", "density"]],
                              on=name_col, how="left")
    if not zaap_scores.empty:
        merged = merged.merge(zaap_scores[[name_col, "zaap_score", "coverage_pct"]],
                              on=name_col, how="left")
    if not access_scores.empty:
        _merge_access = [name_col, "access_score"]
        if "avg_distance_km" in access_scores.columns:
            _merge_access.append("avg_distance_km")
        merged = merged.merge(access_scores[_merge_access], on=name_col, how="left")
    if not coop_scores.empty:
        _merge_coop = [name_col, "coop_score", "white_zone_pct"]
        if "n_cooperatives" in coop_scores.columns:
            _merge_coop.append("n_cooperatives")
        merged = merged.merge(coop_scores[_merge_coop], on=name_col, how="left")

    for col in ["density_score", "zaap_score", "access_score", "coop_score"]:
        if col in merged.columns:
            merged[col] = merged[col].fillna(50.0)

    # ── Exposer coop_count et avg_distance_km ────────────────────────
    ...
    # ── Classification 5 classes RdYlGn (labels inversés) ───────────
    try:
    ...
    # ── Champs de sortie ─────────────────────────────────────────────
    merged["name_en"] = merged[name_col].map(REGION_NAMES_EN) if level == "region" else merged[name_col]
    ...
    merged["interpretation"] = merged.apply(
        lambda r: (
            f"PRIORITÉ MAXIMALE — {r[name_col]} : score composite de "
            f"{r['synthesis_score']:.1f}/100. Opportunité d'investissement "
            f"multisectorielle (densité, ZAAP, accessibilité, coopératives)."
            if pd.notna(r.get("synthesis_class")) and r["synthesis_class"] >= 4
            else (
                f"PRIORITÉ MOYENNE — {r[name_col]} : score composite de "
                f"{r['synthesis_score']:.1f}/100. Des améliorations ciblées "
                f"sont recommandées."
                if pd.notna(r.get("synthesis_class")) and r["synthesis_class"] == 3
                else (
                    f"BIEN DESSERVI — {r[name_col]} : score composite de "
    ...
    metadata = {
        ...
        "level": level,
        "method": (
            f"Superposition pondérée de 4 analyses spatiales au niveau {noun} : "
        ...
        "method_en": (
            f"Weighted overlay of 4 spatial analyses at {noun_en} level: "
        ...
        "weights": WEIGHTS,
        "inputs": [
            f"density{suffix}.geojson",
            f"zaap_coverage{suffix}.geojson",
            f"accessibility{suffix}.geojson",
            f"cooperative_network{suffix}.geojson",
        ],
        "output_fields": {
            name_col: f"Nom de la {noun}",
            "synthesis_score": f"Score composite pondéré (0-100, élevé = bien desservi pour la {noun})",
            "synthesis_class": f"Classe de synthèse (1-5, 5 = prioritaire pour la {noun})",
            "investment_priority_score": f"Score de priorité d'investissement (0-100, élevé = prioritaire pour la {noun})",
        ...
        "interpretation_note": (
            f"Le score composite (0-100) intègre 4 dimensions de l'accès aux "
            f"services agricoles au niveau {noun}. Plus le score est faible, plus la {noun} est "
            f"prioritaire pour les investissements. Les {noun_pl} en rouge (classe 5) "
            f"cumulent des déficits sur plusieurs dimensions et constituent des "
            "opportunités d'investissement prioritaires pour l'intervention."
        ),
        "interpretation_note_en": (
            f"The composite score (0-100) integrates 4 dimensions of agricultural "
            f"service access at {noun_en} level. The lower the score, the more the {noun_en} is a priority "
            f"for investment. {noun_pl_en.capitalize()} in red (class 5) cumulate deficits across "
...
def run(level: str = "region", logger: PipelineLogger | None = None) -> tuple[Path, Path]:
    ...
    logger.section(f"ANALYSE 5 : SYNTHÈSE PONDÉRÉE ({level_label})")

    logger.step("Calcul de la synthèse pondérée...")
    result, metadata = compute_synthesis(logger, level)
    ...
    geojson_path = Path(f"data/public/analysis/synthesis{suffix}.geojson")
    ...
    metadata_path = Path(f"data/public/analysis/metadata/synthesis{suffix}.json")
    ...
    logger.done(f"Synthèse ({level_label}) terminée")
